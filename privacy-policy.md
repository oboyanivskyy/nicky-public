...
When you use Nicky, the App may store the following on your device:

- **Transactions** — amounts, income/expense type, category, optional notes, dates, and optional recurring schedule details
- **Budget settings** — monthly spending limits and category caps you configure
- **Custom categories** — names, icons, and colors you create
- **Preferences** — such as your selected currency

This data is saved using on-device storage (AsyncStorage) and remains on your phone or tablet unless you delete it, erase all app data, or uninstall the App.
...
## Features You Control: Export, Import, and Sharing

Nicky includes optional tools that **you** initiate:

- **Export transactions (CSV)** — creates a file you can share through your device's share sheet (e.g. Messages, Mail, cloud storage apps)
- **Export backup (JSON)** — creates a backup file you can save or share
- **Import transactions (CSV)** or **Restore backup (JSON)** — lets you select a file from your device or cloud provider

When you use these features, data leaves the App only through the destination **you** choose (for example, an email app or iCloud Drive). We do not receive that export unless you send it to us directly (such as by email for support). Any third-party service you use to store or transmit exported files is governed by that service's privacy policy.

---

## Deleting Your Data

You can remove your Nicky data at any time:

- Delete individual transactions inside the App
...
We may update this Privacy Policy from time to time. When we do, we will revise the "Last updated" date above. If changes are material, we may provide additional notice in the App or on the page where this policy is hosted. Continued use of the App after an update means you accept the revised policy.
